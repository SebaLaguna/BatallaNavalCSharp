...
public abstract class BaseHandler : IHandler
{
    ...
    protected abstract void InternalHandle(Message message, out string response, out ReplyKeyboardMarkup reply); // Agrego argumento reply
    ...
    protected virtual bool CanHandle(Message message)
    {
        ...
        // Del mensaje enviado se toma la primera palabra y se compara con las palabras clave
        string[] receivedMessage = message.Text.Split(" ");
        string firstWord = receivedMessage[0];

        return this.Keywords.Any(keyword => keyword.Equals(firstWord, StringComparison.InvariantCultureIgnoreCase));
    }
    ...
    public IHandler Handle(Message message, out string response, out ReplyKeyboardMarkup reply)
    {
        if (this.CanHandle(message))
        {
            this.InternalHandle(message, out response, out reply); // agrego argumento reply 
            return this;
        }
        else if (this.Next != null)
        {
            return this.Next.Handle(message, out response, out reply); // agrego argumento reply
        }
        else
        {
